Ext.define('app.view.HateMap', {
	...
	addUserMarker: function(location, icon) {
		...
		if(this.marker == null) {
	    	...
	    	this.marker.setIcon("assets/images/blue_dot_circle.png");
	    	this.marker.setPosition(location);
	    }

    	me.infoWindow.setContent('<div style="overflow:hidden; padding-bottom:0px"><div style="width:220px; font-size:2.0em"><b>300+ </b><img src="assets/images/HatesMapLogo.png" /><br/><b> near you!</b></div><br/></div>');
    ...
    addHateMarker: function(location, icon, obj) {
        ...
        var strDesc = obj.data.desc;
        var strUrl = obj.data.url;
    ...
    iterateMe: function(i, showSingleHate) {
    	var me = this;
    	var rec = Ext.getStore('Hates');
		arr = rec.data.all;
    	pos = new google.maps.LatLng(arr[i].data.lat, arr[i].data.long);
		var icon = "assets/images/hate-unit.png";
		//console.log(arr[i].data.weight);
		if(arr[i].data.weight < 3) icon = "assets/images/hate-unit.png";
		else if(arr[i].data.weight < 9 && arr[i].data.weight >= 3) icon = "assets/images/hate-unit.png";
		else if(arr[i].data.weight < 11 && arr[i].data.weight >= 9) icon = "assets/images/hate-unit-max2.png";
		
		if(showSingleHate != null) {
			console.log("HDHDHD");
			pos = new google.maps.LatLng(arr[showSingleHate].data.lat, arr[showSingleHate].data.long);
			me.addHateMarker( pos, "assets/images/hate-unit-max.png",  arr[showSingleHate]);
		}
